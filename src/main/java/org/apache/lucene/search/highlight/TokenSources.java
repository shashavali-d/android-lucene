...
public class TokenSources {
   ...
   * {@link Document} to retrieve the TokenStream.
   * This is useful when you already have the document, but would prefer to use
   * the vector first.
   *
   * @param reader The {@link IndexReader} to use to try
   ...
   * @return The {@link TokenStream} for the
  ...
  /** Simply calls {@link #getTokenStream(Terms)} now. */
  @Deprecated // maintenance reasons LUCENE-6445
   ...
   * are not included in the termvector, return null.  See {@link #getTokenStream(Terms)}
